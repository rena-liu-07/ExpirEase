...
def analyze_image(image_path):
    ...
    prompt = {
        "text": (
            ...
            "- The item name\n"
            "- The labeled expiration date (if present) in the format %Y-%m-%d \n"
            ...
            "Item: <item>\nExpiration: <day>\nCategory: <category>\n"
            "List each item on a new line. Each container can be counted as one item."
        )
    }
    ...
    for entry in items:
        ...
        if entry.get('expiration'):
            try:
                ...
                try:
                    days_left = int(entry['expiration'])
                except Exception:
                    days_left = 0
        else:
            days_left = 7
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        image_paths = ["pictures/produce_test.jpg"]
